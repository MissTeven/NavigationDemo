 ...
 * on 2017/3/31.
 */
public class JWheelRecyclerView extends RecyclerView {
    ...
    public JWheelRecyclerView(Context context) {
        this(context, null);
    }

    public JWheelRecyclerView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }
    ...
    private void init(AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray ta = getContext().obtainStyledAttributes(attrs, R.styleable.JWheelRecyclerView);
            visibleCount = ta.getInteger(R.styleable.JWheelRecyclerView_visibleCount, 5);// 默认显示5个
            itemHeight = ta.getDimensionPixelOffset(R.styleable.JWheelRecyclerView_itemHeight, getResources().getDimensionPixelSize(R.dimen.wheel_recycler_view_item_height));
            wheelWidthWeight = ta.getInteger(R.styleable.JWheelRecyclerView_wheelWidthWeight, 1);// 默认是1
            ta.recycle();
        } else {
            visibleCount = 5;
            itemHeight = getResources().getDimensionPixelSize(R.dimen.wheel_recycler_view_item_height);
            wheelWidthWeight = 1;
        }
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        int screenW = getScreenW(getContext());
